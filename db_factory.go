...
type dbCacheMap struct {
	init sync.Once          // 用来保证Map只会初始化一次。
	m    sync.Mutex         // 用于确保同一个连接的创建过程不并发（TODO: 后续要考虑锁的粒度，理论上对连接串加锁即可）。
	dbs  map[string]*sql.DB // 存放已经初始化的数据库对象，key为连接字符串，value为sql.DB对象。
}
...
func getDb(ctx context.Context, driverName string, connectionString string) (*sql.DB, error) {
	dbCache.init.Do(func() {
		dbCache.dbs = make(map[string]*sql.DB) // 确保只延迟初始化一次CacheMap。
	})
	...
	// 初始化可能失败，由于sync.Once在失败后无法再次运行，所以这里不采用sync.Once直接用锁控制。
	dbCache.m.Lock()
