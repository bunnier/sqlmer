...
func Test_parseSqliteNamedSql(t *testing.T) {
	assertNamedSql := func(t *testing.T, inputSql, expectedSql, expectedParams string) {
		namedParsedResult := parseSqliteNamedSql(inputSql)
...
func Test_bindSqliteArgs(t *testing.T) {
	t.Run("basic_map_binding", func(t *testing.T) {
		...
		if fixedSql != wantSql {
			t.Errorf("sqliteDbClient.bindArgs() sql = %v, wantSql %v", fixedSql, wantSql)
		}
		if !reflect.DeepEqual(gotArgs, wantParam) {
			t.Errorf("sqliteDbClient.bindArgs() args = %v, wantParam %v", gotArgs, wantParam)
		}
	})
	...
	t.Run("map_binding_missing_param", func(t *testing.T) {
		...
		if !errors.Is(err, sqlmer.ErrParseParamFailed) {
			t.Errorf("sqliteDbClient.bindArgs() error = %v, wantErr %v", err, sqlmer.ErrParseParamFailed)
		}
	})
