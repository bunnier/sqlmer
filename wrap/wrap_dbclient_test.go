...
func newWrapMysqlClient(t *testing.T) (sqlmer.DbClient, error) {
	dbClient, err := testenv.NewMysqlClient()
	if err != nil {
		t.Fatal(err)
	}

	dbClient = Extend(dbClient, func(sql string, args []any) func(error) {
...
func Test_NewMySqlDbClient(t *testing.T) {
	dbClient, err := newWrapMysqlClient(t)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_internalDbClient_Scalar(t *testing.T) {
	mysqlClient, err := newWrapMysqlClient(t)
	if err != nil {
		t.Fatal(err)
	}
